...
### 7. Explore Chainlink Data Feeds

Scaffold-Stylus includes a Chainlink Data Feed integration example that demonstrates how to fetch real-time price data from Chainlink oracles. This feature is available on **Arbitrum Sepolia** and **Arbitrum Mainnet** networks.

The example fetches ARB/USD price data and includes:

- A custom hook (`useChainlinkArbUsdFeed`) for reading data from the Chainlink price feed
- External contract configuration for both Arbitrum Sepolia and Mainnet
- A dedicated page at `/link-data-feed` to view live price data

To use the Chainlink integration:

1. Make sure you're connected to either Arbitrum Sepolia or Arbitrum Mainnet
2. Navigate to the "Chainlink Data Feed" page in the app
3. View real-time ARB/USD price data fetched from Chainlink oracles

**Note:** Chainlink data feeds are only available on Arbitrum Sepolia and Arbitrum Mainnet networks. The local development network does not support this feature. You would have to fork these two network to be able to use them locally.
