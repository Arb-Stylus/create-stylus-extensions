...
use openzeppelin_stylus::{
    token::erc721::{
        self,
        extensions::{
            Erc721Metadata, Erc721UriStorage, IErc721Burnable, IErc721Metadata, IErc721UriStorage,
        },
        Erc721, IErc721,
    },
    ...
    fn mint(&mut self, to: Address) -> Result<(), erc721::Error> {
        ...
        match self.erc721._mint(to, token_id) {
            Ok(()) => {
                ...
                self.uri_storage._set_token_uri(token_id, String::from(uri));
                Ok(())
            }
            Err(e) => Err(e),
        }
